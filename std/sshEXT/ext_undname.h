...
static ssh_ccs nameTable[] =
{
	" new", " delete", "=", ">>", "<<", "!", "==", "!=", "[]", "operator", "->", "*", "++", "--", "-", "+", "&", "->*", "/", "%", "<", "<=", ">", ">=", ",", "()", "~", "^", "|", "&&", "||", "*=",
	"+=", "-=", "/=", "%=", ">>=", "<<=", "&=", "|=", "^=", "`vftable'", "`vbtable'", "`vcall'", "`typeof'", "`local static guard'", "`string'", "`vbase destructor'", "`vector deleting destructor'",
	"`default constructor closure'", "`scalar deleting destructor'", "`vector constructor iterator'", "`vector destructor iterator'", "`vector vbase constructor iterator'",
	"`virtual displacement map", "`eh vector constructor iterator'", "`eh vector destructor iterator'", "`eh vector vbase constructor iterator'", "`copy constructor closure'",
	"`udt returning'", "`EH", "`RTTI", "`local vftable'", "`local vftable constructor closure'", " new[]", " delete[]", "`omni callsig'", "`placement delete closure'", "`placement delete[] closure'", ""
};
static ssh_ccs ehTable[] =
{
	" Ptr to Member Data'",
	" Catchable Type'",
...
#define UNDNAME_COMPLETE                (0x00000000)    // Enable full undecoration
#define UNDNAME_NO_LEADING_UNDERSCORES  (0x00000001)    // Remove leading underscores from MS extended keywords
#define UNDNAME_NO_MS_KEYWORDS          (0x00000002)    // Disable expansion of MS extended keywords
#define UNDNAME_NO_FUNCTION_RETURNS     (0x00000004)    // Disable expansion of return type for primary declaration
#define UNDNAME_NO_ALLOCATION_MODEL     (0x00000008)    // Disable expansion of the declaration model
#define UNDNAME_NO_ALLOCATION_LANGUAGE  (0x00000010)    // Disable expansion of the declaration language specifier
#define UNDNAME_NO_MS_THISTYPE          (0x00000020)    // Disable expansion of MS keywords on the 'this' type for primary declaration
#define UNDNAME_NO_CV_THISTYPE          (0x00000040)    // Disable expansion of CV modifiers on the 'this' type for primary declaration
#define UNDNAME_NO_THISTYPE             (0x00000060)    // Disable all modifiers on the 'this' type
#define UNDNAME_NO_ACCESS_SPECIFIERS    (0x00000080)    // Disable expansion of access specifiers for members
#define UNDNAME_NO_THROW_SIGNATURES     (0x00000100)    // Disable expansion of 'throw-signatures' for functions and pointers to functions
#define UNDNAME_NO_MEMBER_TYPE          (0x00000200)    // Disable expansion of 'static' or 'virtual'ness of members
#define UNDNAME_NO_RETURN_UDT_MODEL     (0x00000400)    // Disable expansion of MS model for UDT returns
#define UNDNAME_32_BIT_DECODE           (0x00000800)    // Undecorate 32-bit decorated names
#define UNDNAME_NAME_ONLY               (0x00001000)    // Crack only the name for primary declaration; return just [scope::]name.  Does expand template params
#define UNDNAME_TYPE_ONLY               (0x00002000)    // Input is just a type encoding; compose an abstract declarator
#define UNDNAME_NO_RETURN_ARRAY			(0x00008000)    // îòêëş÷èòü âîçâğàò îïğåäåëåíèÿ ìàññèâà
#define UNDNAME_NO_RETURN_CONSTABLES	(0x00010000)	// îòêëş÷èòü ìîäèôèêàòîğ const
#define UNDNAME_NO_RETURN_VOLATILES		(0x00020000)	// îòêëş÷èòü ìîäèôèêàòîğ volatile
#define UNDNAME_NO_RETURN_MUTABLES		(0x00040000)	// îòêëş÷èòü ìîäèôèêàòîğ mutable
